<template>
  <div>
      ...
      <div class="padding-add">
        <el-form ref="tableFrom" :model="tableFrom" :label-width="labelWidth" label-position="right"
          ...
          <el-form-item label="活动状态：">
            <el-select placeholder="请选择" clearable v-model="tableFrom.status" @change="searchs"
    ...
    <el-card :bordered="false" shadow="never" class="ivu-mt mt16">
      <el-button v-auth="['marketing-store_seckill-create']" type="primary" v-db-click @click="add">添加秒杀活动</el-button>
      <el-table :data="tableList" v-loading="loading" highlight-current-row no-userFrom-text="暂无数据"
        ...
        <el-table-column label="活动时段" min-width="200">
          <template slot-scope="scope">
            <!-- <div v-for="(item, index) in scope.row.times_list">{{ item }}</div> -->
            <el-tag class="m2" v-for="item in scope.row.times_list" effect="plain">
              {{ item }}
            </el-tag>
          </template>
        </el-table-column>
